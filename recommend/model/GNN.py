...
class GNN(object):
    ...
    def _build_inputs(self):
        ...
        self.e = tf.keras.Input(dtype=tf.int64, shape=[None], name='e')
        self.pos_e = tf.keras.Input(dtype=tf.int64, shape=[None], name='pos_e')
        self.neg_e = tf.keras.Input(dtype=tf.int64, shape=[None], name='neg_e')

            # inputs for kg
        self.h = tf.keras.Input(dtype=tf.int64, shape=[None], name='h')
        self.r = tf.keras.Input(dtype=tf.int64, shape=[None], name='r')
        self.pos_t = tf.keras.Input(dtype=tf.int64, shape=[None], name='pos_t')
        self.neg_t = tf.keras.Input(dtype=tf.int64, shape=[None], name='neg_t')
    ...
    def _build_weights(self, embedding_type: str) -> None:
        ...
        initializer = tf.keras.initializers.GlorotUniform(seed=2021)
    ...
    def _create_gcn_embed(self) -> tf.Tensor:
        ...
        for k in range(self.n_layer):
            ...
            temp_embed = []
            if self.n_fold == 1:
                neighbor_embedding = tf.sparse.sparse_dense_matmul(A_fold_hat[0], pre_embedding, name='gcn_neighbor_{}'.format(k))
            else:
                for f in range(self.n_fold):
                    temp_embed.append(tf.sparse.sparse_dense_matmul(A_fold_hat[f], pre_embedding))
    ...
    def _create_graphsage_embed(self) -> tf.Tensor:
        ...
        for k in range(self.n_layer):
            ...
            temp_embed = []
            if self.n_fold == 1:
                neighbor_embedding = tf.sparse.sparse_dense_matmul(A_fold_hat[0], pre_embedding, name='sage_neighbor_{}'.format(k))
            else:
                for f in range(self.n_fold):
                    temp_embed.append(tf.sparse.sparse_dense_matmul(A_fold_hat[f], pre_embedding))
            ...
            print(f"pre_embedding: {pre_embedding}")
            # print(f"self.mess_dropout[k]: {self.mess_dropout[k]}")
            # pre_embedding = tf.nn.dropout(pre_embedding, 1 - self.mess_dropout[k], name='sage_dropout_{}'.format(k))
    ...
    def _create_bi_inter_embed(self) -> tf.Tensor:
        ...
        for k in range(self.n_layer):
            ...
            temp_embed = []
            if self.n_fold == 1:
                neighbor_embedding = tf.sparse.sparse_dense_matmul(A_fold_hat[0], pre_embedding, name='bi_neighbor_{}'.format(k))
            else:
                for f in range(self.n_fold):
                    temp_embed.append(tf.sparse.sparse_dense_matmul(A_fold_hat[f], pre_embedding))
